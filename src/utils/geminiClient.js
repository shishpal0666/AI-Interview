...
async function callGemini(prompt, options = {}) {
  ...
  if (options.responseMimeType !== undefined) body.generationConfig = { ...body.generationConfig, responseMimeType: options.responseMimeType };
  if (options.responseSchema !== undefined) body.generationConfig = { ...body.generationConfig, responseSchema: options.responseSchema };
...
async function callGeminiInternal(prompt, options = {}) {
  try {
    ...
    const msg = (err && err.message) || String(err) || 'Gemini request failed';
...
export async function generateQuestions(difficulties = ['Easy', 'Easy', 'Medium', 'Medium', 'Hard', 'Hard'], options = {}) {
  ...
  let raw = null;
  try {
    raw = await callGeminiInternal(userPrompt, { temperature: 0.2, maxOutputTokens: 512, responseMimeType: 'application/json', responseSchema: schema, ...options });
  } catch (err) {
...
export async function evaluateAnswers(questions = [], answers = [], options = {}) {
  ...
  const schema = {
    ...
    properties: {
      evaluations: {
        type: 'ARRAY',
        description: 'An array of evaluations, one for each question-answer pair.',
        items: {
          type: 'OBJECT',
          properties: {
            score: { type: 'NUMBER', description: 'A numerical score for the answer, out of 10.' },
            feedback: { type: 'STRING', description: "Brief feedback on the answer's correctness." }
          },
...
export async function fetchQuestion(difficulty = 'Easy', options = {}) {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') return String(parsed.question || JSON.stringify(parsed));
